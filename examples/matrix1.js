...
// var e1 = document.getElementById("cs");
//                                     var c1 = e1.options[e1.selectedIndex].value; 
//                                     return colour_map[c1](d.z);
...
function fun(p)
{
  ...
  .append("g")
    ...
    for(i=0;i<n;i++)
    {
      if(orders[corder][i]==p.x)
        break;
    }
    for(j=0;j<n;j++)
    {
      if(orders[corder][j]==p.y)
        break;
    }
    console.log("i: "+i);
    var temp1=i;
    var temp2=j;
    for(i=temp1;i<(temp1+parseInt(len));i++)
    {
      j1=0;
      console.log("p.y: "+p.y);
      console.log("p.y+len: "+(parseInt(p.y)+parseInt(len)));
      for(j=temp2;j<(temp2+parseInt(len));j++)
      {
...
function smaller_matrix(matrix)
{
  ...
  var i,j;
  var mat_new=[];
  console.log("to much: "+Math.floor(matrix.length/10));
...
function matrix2(matrix)
{
  console.log("inside matrix2:");
    ...
    obj1["nodes"]=[];
    obj1["links"]=[];
    for(t1=0;t1<matrix.length;t1++)
    {
      temp={};
      temp["name"]=t1.toString();
      obj1["nodes"].push(temp);
    }
    for(t1=0;t1<matrix.length;t1++)
    {
      for(t2=0;t2<matrix.length;t2++)
      {
        temp={};
        temp["source"]=t1;
        temp["target"]=t2;
        temp["value"]=matrix[t1][t2].z;
        obj1["links"].push(temp);
      }
    }
  ...
  console.log("Precompute the orders");
    var orders = {
  name: d3.range(matrix.length).sort(function(a, b) { console.log("hi"); return d3.ascending(parseInt(nodes[a].name), parseInt(nodes[b].name)); }),
  count: d3.range(matrix.length).sort(function(a, b) { return nodes[b].count - nodes[a].count; }),
  group: d3.range(matrix.length).sort(function(a, b) {
      var x = nodes[b].group - nodes[a].group;
      return (x != 0) ?  x : d3.ascending(nodes[a].name, nodes[b].name);
  }),
  leafOrder: computeLeaforder,
